...
# Config
$baseFolder = $rootDir
if (-not $baseFolder) { $baseFolder = Split-Path -Parent $MyInvocation.MyCommand.Path }

# Select configuration profile source directory (root configs vs Cloudflare_Profiles)
function Read-ProfileSource {
    param([string]$baseFolder)
    $cfFolder = Join-Path $baseFolder "Cloudflare_Profiles"
    $hasCf = Test-Path $cfFolder

    while ($true) {
        Write-Host ""
        Write-Host "Select configuration profile source directory:" -ForegroundColor Cyan
        Write-Host "  [1] Root directory (standard general*.bat configs)" -ForegroundColor Gray
        if ($hasCf) {
            Write-Host "  [2] Cloudflare_Profiles directory (generated custom profiles)" -ForegroundColor Gray
        } else {
            Write-Host "  [2] Cloudflare_Profiles directory (NOT FOUND)" -ForegroundColor DarkGray
        }
        ...
        switch ($choice) {
            '1' { return $baseFolder }
            '2' {
                if ($hasCf) { return $cfFolder }
                else { Write-Host "Directory not found." -ForegroundColor Yellow }
            }
...
$targetDir = Read-ProfileSource -baseFolder $baseFolder
...
if (-not $batFiles -or $batFiles.Count -eq 0) {
    Write-Host "[ERROR] No *.bat files found in: $targetDir" -ForegroundColor Red
    Write-Host "Press any key to exit..." -ForegroundColor Yellow
    [void][System.Console]::ReadKey($true)
    exit 1
}
...
try {
    ...
    # Cleanup - stop previous winws and wait for it to fully release (WinDivert driver)
    Stop-Zapret
    $stopWaitMs = 0
    while ((Get-Process -Name "winws" -ErrorAction SilentlyContinue) -and ($stopWaitMs -lt 3000)) {
        Start-Sleep -Milliseconds 200
        $stopWaitMs += 200
    }
    Start-Sleep -Milliseconds 500  # extra buffer for driver handle release
    ...
    # Wait for winws.exe to actually appear and stay alive (instead of a blind fixed sleep)
    $winwsReady = $false
    $waitMs = 0
    $maxWaitMs = 10000
    while ($waitMs -lt $maxWaitMs) {
        ...
        if ($winwsProc) {
            # Make sure it didn't crash immediately - give it one more check after a short delay
    ...
    if (-not $winwsReady) {
        Write-Host "  [WARN] winws.exe did not start (or crashed immediately) for this config." -ForegroundColor Red
        Write-Host "  [WARN] Skipping tests for '$($file.Name)' - results would be invalid (no active bypass)." -ForegroundColor Red
        if (-not $proc.HasExited) { Stop-Process -Id $proc.Id -Force -ErrorAction SilentlyContinue }
        Stop-Zapret
        continue
    }

    # Extra settle time after confirming winws is alive
